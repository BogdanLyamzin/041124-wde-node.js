...
export const findUserOrAdmin = async (query) => {
...
export const registerUser = async (payload) => {
  ...
  if(user) throw HttpError(401, "Email already exist");
...
export const restoreUserPassword = async (email) => {
  const user = await findUserOrAdmin({ email });
  if (!user) throw HttpError(401, "User not found");
...
export const refreshTokens = async ({refreshToken})=> {
  ...
  return await Session.findByIdAndUpdate({_id: session._id}, newSession);
}